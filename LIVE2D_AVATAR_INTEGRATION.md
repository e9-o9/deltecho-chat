# Live2D Avatar Integration with Deep Tree Echo Chat Interface

## Overview
This document describes the integration of Live2D Cubism Avatar with the Deep Tree Echo cognitive AI system in the DeltaChat interface.
...
Main React component that displays the Live2D avatar and manages its visual state.
...
### App.tsx
Added `DeepTreeEchoAvatarProvider` to the component tree to make avatar context available globally:
...
### Internal
- `@deltecho/avatar` - Live2D avatar management and rendering
- `@deltecho/cognitive` - Cognitive state types (compatible types defined locally)
- `CognitiveBridge` - Cognitive orchestrator for state

### External
- React (hooks: useState, useEffect, useCallback, useRef, useContext)
- Live2D Cubism SDK (via @deltecho/avatar)
- PixiJS (via pixi-live2d-display-lipsyncpatch)
...
- [Cognitive Bridge Documentation](./packages/frontend/src/components/DeepTreeEchoBot/CognitiveBridge.ts)
